...
async def after_command(bot, cid):	
	game = get_game(cid)
...
async def after_ruta_achicada(bot, cid, uid):
	#sleep(3)
	game = get_game(cid)
	...
	if not game.board.cartasExplorationActual:
		...
		await bot.send_message(cid, "Exploracion Actual no tiene cartas. Se cambia a %s" % tiempo)
		
		if player.food != 0:
			await bot.send_message(cid, "Se pierde uno de comida")
			command_losefood(bot, [0, cid, uid])
		else:
			await bot.send_message(cid, "Como no hay comida alguien tiene habre. (-1 vida)")
...
async def command_newgame_lost_expedition(update: Update, context: CallbackContext):
	bot = context.bot
	...
	if uid in ADMIN:
		try:
			...
			if game:
				...
				await bot.send_message(cid, "Se creo el juego y el usuario")
				await save(bot, game.cid)
				MainController.init_game(bot, game)

		except Exception as e:
			await bot.send_message(cid, 'Error '+str(e))
		
async def command_drawcard(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
...
async def command_showhand(bot, args):
	...
	if uid in ADMIN:
		...
		#cid = '-1001206290323'
		if not player.hand:
			await bot.send_message(cid, "El jugador no tiene cartas")
		else:
			
			showImages(bot, cid, player.hand, "Mano del Jugador")
		
async def command_showskills(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
...
async def command_increase_progreso(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
	...
	if uid in ADMIN:
		...
		if game.board.progreso == game.board.objetivoprogreso:
			await bot.send_message(cid, "Ganaste")
		else:
			await bot.send_message(cid, "Estas a %s de distancia, el objetivo es 9" % game.board.progreso)
		await after_command(bot, cid)
		'''
		player = game.playerlist[uid]
		#cid = '-1001206290323'
...
async def command_losebullet(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
	...
	if uid in ADMIN:
		...
		#cid = '-1001206290323'
		player.bullets -= 1;		
		await bot.send_message(cid, "Se ha perdido una bala")
		#ommand_showstats(bot, update)
		await after_command(bot, cid)
		
async def command_gainbullet(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
	...
	if uid in ADMIN:
		...
		#cid = '-1001206290323'
		player.bullets += 1;
		await bot.send_message(cid, "Se ha ganado una bala")
		#ommand_showstats(bot, update)
		await after_command(bot, cid)
		
async def command_losefood(bot, args):
	cid, uid = args[1], args[2]	
	if uid in ADMIN:
		game = get_game(cid)
		if not game:
		...
		#cid = '-1001206290323'
		player.food -= 1
		await bot.send_message(cid, "Se ha perdido uno de comida")
		#ommand_showstats(bot, update)
		await after_command(bot, cid)
		
async def command_gainfood(bot, args):
	cid, uid = args[1], args[2]
	if uid in ADMIN:
		game = get_game(cid)
		if not game:
		...
		#cid = '-1001206290323'
		player.food += 1
		await bot.send_message(cid, "Se ha ganado uno de comida")
		#ommand_showstats(bot, update)
		await after_command(bot, cid)

async def command_lose_life(bot, args):
	cid, uid = args[1], args[2]
...
async def command_gain_life(bot, args):
	cid, uid = args[1], args[2]
	if uid in ADMIN:
		game = get_game(cid)
		if not game:
...
async def command_vida_explorador_campero(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	cid, uid = update.message.chat_id, update.message.from_user.id	
	if uid in ADMIN:
...
async def command_vida_explorador_brujula(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	cid, uid = update.message.chat_id, update.message.from_user.id	
	if uid in ADMIN:
...
async def command_vida_explorador_hoja(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	cid, uid = update.message.chat_id, update.message.from_user.id	
	if uid in ADMIN:
...
async def command_add_exploration(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
	...
	if uid in ADMIN:
		...
		if game.board.state.esdedia:			
			...
			await bot.send_message(cid, "Se ha agregado la carta a la ruta y se ha ordenado la ruta")
		else:
			await bot.send_message(cid, "Se ha agregado la carta al final de la ruta")
		#command_showhand(bot, update)
		#command_show_exploration(bot, update)		

async def command_add_exploration_first(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
	...
	if uid in ADMIN:
		...
		await bot.send_message(cid, "Se ha agregado la carta al principio de la ruta")
		await after_command(bot, cid)
		if game.board.state.esdedia:
			command_sort_exploration_rute(bot, args)
		#command_showhand(bot, update)
...
async def draw_card_cartasAventura(game, destino):
	...
	if not game.board.cartasAventura:
		...
		if game.board.state.amount_shuffled == 1:
			await bot.send_message(cid, "Se ha mezclado el mazo y se debe consumir 1 de comida")
			#for uid in game.playerlist:
			#	player = game.playerlist[uid]
		else:
			await bot.send_message(cid, "Se ha perdido la partida porque se ha mezclado el mazo dos veces. /delete")
		'''

async def command_peek_deck(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
...
async def command_add_rute_option(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
...
async def command_add_exploration_deck(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
	...
	if uid in ADMIN:
		...
		await bot.send_message(cid, "Se ha agregado %s cartas al final de la ruta desde el mazo" % cantidad)
...
async def command_add_exploration_deck_first(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
...
async def command_show_exploration(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
...
async def command_sort_exploration_rute(bot, args):
	...
	if uid in ADMIN:
		...
		await after_command(bot, cid)
		#await bot.send_message(cid, "Las cartas de ruta han sido ordenadas.")

async def command_swap_exploration(bot, args):
	cid, uid = args[1], args[2]
	if uid in ADMIN:
		game = get_game(cid)
		if not game:
		...
		if len(args) < 2:
			await bot.send_message(cid, "Se tienen que ingresar 2 argumentos")
			return			
...
async def command_remove_exploration(bot, args):
	cid, uid = args[1], args[2]
	if uid in ADMIN:
		game = get_game(cid)
		if not game:
		...
		try:			
			...
			await bot.send_message(cid, "La carta se ha eliminado de la ruta")
			...
			if str(e) == "pop index out of range":
				...
				if game.board.state.comando_pedido:
					await bot.send_message(cid, "Se ha intentado sacar una carta que no existe, considero ejecutada la accion.")
					await after_command(bot, cid)
				
			else:
				await bot.send_message(cid, "El remover carta de exploracion ha fallado debido a: "+str(e))
			

async def command_remove_last_exploration(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
	...
	if uid in ADMIN:
		...
		if len(game.board.cartasExplorationActual) == 1:
			await bot.send_message(cid, "No se puede quitar la ultima carta de exploración, considero ejecutada la acción.")
			await after_command(bot, cid)
		else:
...
async def command_resolve_exploration(update: Update, context: CallbackContext):
	bot = context.bot
	cid, uid = update.message.chat_id, update.message.from_user.id	
	if uid in ADMIN:
...
async def command_gain_skill(bot, args):
	cid, uid = args[1], args[2]
	if uid in ADMIN:
		game = get_game(cid)
		if not game:
		...
		await bot.send_message(cid, "La carta de la ruta ha sido obtenida como skill")
		after_ruta_achicada(bot, cid, uid)
		await after_command(bot, cid)
		#command_show_exploration(bot, update)
		
async def command_use_skill(bot, args):
	cid, uid = args[1], args[2]
	if uid in ADMIN:
		game = get_game(cid)
		player = game.playerlist[uid]
		...
		if args and args[0] == -1:
			...
			if not player.skills:
				...
				await bot.send_message(cid, "Elija una carta de skill:", reply_markup=btnMarkup)
				return "Esperar"
		else:
			#cid = '-1001206290323'
			# Defecto saco la de la izquierda
...
async def command_sort_hand(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
...
async def command_lose_camp(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	#log.info(args)
	try:
...
async def command_lose_compass(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	#log.info(args)
	try:
...
async def command_lose_leaf(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	#log.info(args)
	try:
...
async def command_lose_explorer(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	try:
		cid, uid = update.message.chat_id, update.message.from_user.id
...
async def get_base_data2(cid, uid):
...
async def command_rules(update: Update, context: CallbackContext):
	...
	await bot.send_message(cid, "Read the official Secret Hitler rules:", reply_markup=rulesMarkup)
		...
		if game:			
			if not game.dateinitvote:
				...
				if elapsed > datetime.timedelta(minutes=1):
					...
					await bot.send_message(cid, "Five minutes must pass to see call to vote") 
		else:
...
async def command_worflow(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	cid, uid = update.message.chat_id, update.message.from_user.id
	game, player = get_base_data2(cid, uid)	
...
async def execute_actions(bot, cid, uid):
	...
	if game is not None:
		...
		if tipo_accion_actual == "opcional":
			#await bot.send_message(cid, "Es una accion opcional. El indice es %s" % str(index_opcion_actual))
		...
		if index_opcion_actual != 0:
			...
			#await bot.send_message(cid, "Realizando comando %s/%s de la accion %s" % (str(index_comando_actual), str(len(comandos_opcion_actual)), game.board.state.index_accion_actual))
			...
			if index_comando_actual > len(comandos_opcion_actual):
				...
				if "ejecutar_al_final" in opcion_actual:
					...
					await bot.send_message(cid, "Deberia ejecutar al final el comando %s" % ejecutar_al_final)
				else:
		...
		#except Exception as e:
		#	await bot.send_message(cid, 'No se ejecuto el execute_actions debido a: '+str(e))
	
async def send_choose_buttons(bot, cid, uid, game, opciones_accion_actual):
	#sleep(3)
	strcid = str(game.cid)
	btns = []
	player = game.playerlist[uid]
	...
	btnMarkup = InlineKeyboardMarkup(btns)
	#for uid in game.playerlist:
	await bot.send_message(cid, "Elija una de las opciones:", reply_markup=btnMarkup)

async def get_atribute(restriccion, game, player):
...
async def verify_restriction(atributo, tipo, restriccion):
	if tipo == "len":
		return str(len(atributo)) == restriccion
	elif tipo == "igual":
		return str(atributo) == restriccion
	
async def elegir_opcion_comando(update: Update, context: CallbackContext):
	bot = context.bot	
	#try:		
	callback = update.callback_query
	log.info('elegir_opcion_comando called: %s' % callback.data)	
	regex = re.search(r"(-[0-9]*)\*opcioncomandos\*(.*)\*([0-9]*)", callback.data)
	cid, strcid, opcion, uid, struid = int(regex.group(1)), regex.group(1), regex.group(2), int(regex.group(3)), regex.group(3)	
...
async def elegir_opcion_skill(update: Update, context: CallbackContext):
	bot = context.bot	
	#try:		
	callback = update.callback_query
	log.info('elegir_opcion_comando called: %s' % callback.data)	
	regex = re.search(r"(-[0-9]*)\*opcionskill\*(.*)\*([0-9]*)", callback.data)
	cid, strcid, opcion, uid, struid = int(regex.group(1)), regex.group(1), regex.group(2), int(regex.group(3)), regex.group(3)	
...
async def ejecutar_comando(bot, cid, uid, comando, comando_argumentos, ejecutar_al_final):
	#try:
	log.info('ejecutar_comando called: %s' % comando)
	#await bot.send_message(cid, comando)
	#sleep(3)
	game, player = get_base_data2(cid, uid)
	...
	if tipo_comando == "automatico":
		...
		if "player.hand" in comando["indicacion_argumentos"]:			
			...
			btnMarkup = InlineKeyboardMarkup(btns)
		#for uid in game.playerlist:
		await bot.send_message(cid, comando["indicacion"], reply_markup=btnMarkup)
	else:
...
async def get_list_buttons(uid, lista, strcomando, strcid, callback_comando = 'exe'):
	i = 1
	btns = []
	buttonGroup = []	
...
async def get_player_exploradores_buttons(player, comando, strcid):	
	i = 1
	btns = []
	buttonGroup = []
...
async def increase_count_cartas_deck(bot, game, player):
	game.board.state.count_cartas_deck += 1
	await save(bot, game.cid)
	
async def reset_count_cartas_deck(bot, game, player):
	game.board.state.count_cartas_deck = 0
	await save(bot, game.cid)

async def resolve(bot, cid, uid, game, player):
	if game is not None:
		...
		if not game.board.cartasExplorationActual:
			...
			#await bot.send_message(cid, carta)
...
async def command_resolve_exploration2(update: Update, context: CallbackContext):
	bot = context.bot
	...
	if game:
		...
		await bot.send_message(cid, "No hay juego que resolver")	
