...
async def command_acusar(update: Update, context: CallbackContext):
    ...
    if not game or game.tipo != "SpyFall":
        await bot.send_message(cid, "No hay partida de SpyFall activa aquí.")
        return
    if not game.board:
        await bot.send_message(cid, "La partida no ha comenzado.")
        return
    if uid not in game.playerlist:
        await bot.send_message(cid, "No estás en esta partida.")
        return
    if game.board.state.fase_actual == "Votando":
        await bot.send_message(cid, "⚠️ Ya hay una votación en curso.")
        return
    if game.board.state.fase_actual != "Interrogando":
        await bot.send_message(cid, "No se puede acusar en este momento.")
        return
...
async def callback_spyfall_voto(update: Update, context: CallbackContext):
    ...
    try:
        ...
        await callback.answer(f"Votaste por {accused_player.name}")
        await bot.send_message(
            cid,
            f"✅ *{voter_name}* votó.",
            parse_mode=ParseMode.MARKDOWN
        ...
        if len(game.board.state.votos_acusacion) >= len(game.playerlist):
            await save(bot, game.cid)
            await SpyFallController.resolve_accusation(bot, game)
        else:
            await save(bot, game.cid)
        ...
        try:
            ...
            pass
        await bot.send_message(ADMIN[0], f"SpyFall voto error: {e}")


async def command_adivinar(update: Update, context: CallbackContext):
    ...
    if game.board.state.fase_actual == "Finalizado":
        await bot.send_message(cid, "La partida ya terminó.")
        return
...
async def callback_spyfall_adivinar(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if location_idx >= len(LISTA_LOCALIDADES):
            await callback.answer("Localidad inválida.")
            return
        ...
        if game.board.state.fase_actual == "Finalizado":
            await callback.answer("La partida ya terminó.")
            return
        ...
        if not player.is_spy:
            await callback.answer("Solo el espía puede adivinar.")
            return

        await callback.answer(f"Adivinaste: {guessed_localidad}")
        ...
        if guessed_localidad == real_localidad:
            await SpyFallController.spy_wins(
                bot, game,
                reason="✅ *¡El espía adivinó la localidad correctamente!*"
            )
        else:
            await SpyFallController.non_spy_wins(bot, game, guessed_wrong=True)
        ...
        try:
            ...
            pass
        await bot.send_message(ADMIN[0], f"SpyFall adivinar error: {e}")
