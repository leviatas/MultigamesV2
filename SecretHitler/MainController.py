...
async def start_round(bot, game):        
	...
	await bot.send_message(game.cid, msgtext, ParseMode.MARKDOWN)
...
async def choose_chancellor(bot, game):
	...
	if game.is_debugging:
		...
		await bot.send_message(ADMIN, 'Por favor nomina a tu canciller!', parse_mode=ParseMode.MARKDOWN, reply_markup=chancellorMarkup)      
	else:
		...
		await bot.send_message(game.board.state.nominated_president.uid, msg, parse_mode=ParseMode.MARKDOWN, reply_markup=chancellorMarkup)
...
async def nominate_chosen_chancellor(update: Update, context: CallbackContext):
	bot = context.bot
...
async def vote(bot, game):
	...
	for uid in game.playerlist:
		if not game.playerlist[uid].is_dead and not game.is_debugging:
			...
			await bot.send_message(uid, msg,	reply_markup=voteMarkup, parse_mode=ParseMode.MARKDOWN)


async def handle_voting(update: Update, context: CallbackContext):
	bot = context.bot
	callback = update.callback_query
...
async def count_votes(bot, game):
	...
	if list(game.board.state.last_votes.values()).count("Ja") > (
		...
		await bot.send_message(game.cid, voting_text, ParseMode.MARKDOWN)
		...
		await bot.send_message(game.cid, voting_text)
...
async def draw_policies(bot, game):
	...
	if not game.is_debugging:
		await bot.send_message(game.board.state.president.uid, f"{game.groupName}\nHas robado las siguientes 3 politicas. Cual quieres descartar?",
			reply_markup=choosePolicyMarkup)
	else:
		await bot.send_message(ADMIN, f"{game.groupName} Has robado las siguientes 3 politicas. Cual quieres descartar?",
			reply_markup=choosePolicyMarkup)
...
async def choose_policy(update: Update, context: CallbackContext):
	bot = context.bot
	...
	try:
		...
		if len(game.board.state.drawn_policies) == 3 and uid == game.board.state.president.uid:
			...
			if answer == "veto" :
				...
				await bot.send_message(game.board.state.president.uid,
...
async def pass_two_policies(bot, game):
	...
	if game.board.state.fascist_track == 5 and not game.board.state.veto_refused:
		...
		await bot.send_message(game.board.state.chancellor.uid,
...
async def enact_policy(bot, game, policy, anarchy):
	...
	if not anarchy:
		await bot.send_message(game.cid, "El Presidente %s y el Canciller %s promulgaron una política %s!" % (game.board.state.president.name, game.board.state.chancellor.name, policy))
		...
		await bot.send_message(game.cid, "La política en la cima del mazo ha sido promulgada y es %s" % policy)
...
async def choose_veto(update: Update, context: CallbackContext):
	bot = context.bot
	callback = update.callback_query
	regex = re.search("(-[0-9]*)_(.*)", callback.data)
	cid = int(regex.group(1))
...
async def do_anarchy(bot, game):
	...
	await bot.send_message(game.cid, "ANARCHY!!")
...
async def choose_kill(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if chosen.role == "Hitler":
            ...
            await bot.send_message(chosen.uid, "ESTAS MUERTO " + game.board.state.president.name + " TE HA MATADO")
...
async def choose_choose(update: Update, context: CallbackContext):
...
async def choose_inspect(update: Update, context: CallbackContext):
    ...
    try:
        ...
        await bot.send_message(game.cid, "El Presidente %s ha inspeccionado a %s." % (game.board.state.president.name, chosen.name))
...
async def decide_anarquia(bot, game):
	...
	for uid in game.playerlist:
		if not game.is_debugging:
			if not game.playerlist[uid].is_dead:                      
				Commands.print_board(bot, game, uid)				
				await bot.send_message(uid, "¿Quieres ir a anarquia? (CUIDADO si la mitad de los jugadores elige SI no se espera)", reply_markup=voteMarkup)
		else:
			await bot.send_message(ADMIN, game.board.print_board(game.player_sequence))
			await bot.send_message(ADMIN, "¿Quieres ir a anarquia? (CUIDADO si la mitad de los jugadores elige SI no se espera)", reply_markup=voteMarkup)
			
async def handle_voting_anarquia(update: Update, context: CallbackContext):
	bot = context.bot
	callback = update.callback_query
...
async def count_votes_anarquia(bot, game):
	# La votacion ha finalizado.
	game.dateinitvote = None
	# La votacion ha finalizado.
...
async def set_stats(column_name, value, bot, cid):
	conn = psycopg.connect(
		dbname=url.path[1:],
		user=url.username,
		password=url.password,
	...
	try:
		...
		await bot.send_message(cid, 'No se ejecuto el comandoset_stats debido a: '+str(e))
		conn.rollback()
	conn.close()
		
async def save_game_details(bot, print_roles, game_endcode, liberal_track, fascist_track, num_players):
	conn = psycopg.connect(
		dbname=url.path[1:],
		user=url.username,
		password=url.password,
...
async def change_stats(uid, tipo_juego, stat_name, amount):
...
async def save_player_stats(uid, data):
	conn = psycopg.connect(
		dbname=url.path[1:],
		user=url.username,
		password=url.password,
...
async def load_player_stats(uid):
	conn = psycopg.connect(
		dbname=url.path[1:],
		user=url.username,
		password=url.password,
...
async def end_game(bot, game, game_endcode):
	...
	#await bot.send_message(cid, "Datos a guardar %s %s %s %s %s" % (game.print_roles(), str(game_endcode), str(game.board.state.liberal_track), str(game.board.state.fascist_track), str(game.board.num_players)))
		
	stats = get_stats(bot, cid)	
	if game_endcode == 99:
		if GamesController.games[cid].board is not None:
			await bot.send_message(cid, "Juego cancelado!\n\n%s" % game.print_roles())
		else:
			await bot.send_message(cid, "Juego cancelado!")
		set_stats("cancelgame", stats[5] + 1, bot, cid)
	else:
		if game_endcode == -2:
			await bot.send_message(game.cid, "Juego finalizado! Los fascistas ganaron eligiendo a Hitler como Canciller!\n\n%s" % game.print_roles())
			set_stats("fascistwinhitler", stats[1] + 1, bot, cid)
		if game_endcode == -1:
			await bot.send_message(game.cid, "Juego finalizado! Los fascistas ganaron promulgando 6 políticas fascistas!\n\n%s" % game.print_roles())
			set_stats("fascistwinpolicies", stats[2] + 1, bot, cid)
		if game_endcode == 1:
			await bot.send_message(game.cid, "Juego finalizado! Los liberales ganaron promulgando 5 políticas liberales!\n\n%s" % game.print_roles())
			set_stats("liberalwinpolicies", stats[3] + 1, bot, cid)
		if game_endcode == 2:
			await bot.send_message(game.cid, "Juego finalizado! Los liberales ganaron matando a Hitler!\n\n%s" % game.print_roles())
...
async def showHiddenhistory(bot, game):	
	#game.pedrote = 3
	try:
		...
		for x in game.hiddenhistory:				
			history_text += x + "\n"
		await bot.send_message(game.cid, history_text + remaining_policies, ParseMode.MARKDOWN)
	except Exception as e:
...
async def inform_players(bot, game, cid, player_number):
	...
	for uid in player_ids:
		...
		# Si el jugador tiene una preferencia... async defecto se pone "" y daria [''] como preferencias		
		...
		if not game.is_debugging:
			await bot.send_message(uid, "Tu rol secreto es: %s\nTu afiliación política es: %s" % (role, party))
		else:
			await bot.send_message(ADMIN, "El jugador %s es %s y su afiliación política es: %s" % (game.playerlist[uid].name, role, party))
	...
	for uid in game.playerlist:
		...
		if role == "Fascista":
			...
			if player_number > 6:
				...
				fstring = fstring[:-2]
				if not game.is_debugging:
					await bot.send_message(uid, "Tus compañeros fascistas son: %s" % fstring)
			hitler = game.get_hitler()
			if not game.is_debugging:
				await bot.send_message(uid, "Hitler es: %s" % hitler.name) #Uncoomend on production
		elif role == "Hitler":
			if player_number <= 6:
				fascists = game.get_fascists()
				if not game.is_debugging:
					await bot.send_message(uid, "Tu compañero fascista es: %s" % fascists[0].name)
		elif role == "Liberal":
			pass
		else:
...
async def increment_player_counter(game):
...
async def getGamesByTipo(opcion):
	conn = psycopg.connect(
		dbname=url.path[1:],
		user=url.username,
		password=url.password,
...
async def change_groupname(bot, update):
	cid = update.message.chat.id
	groupname = update.message.chat.title
	game = Commands.get_game(cid)
	game.groupName = groupname
	await bot.send_message(ADMIN, text="El group en {cid} ha cambiado de nombre a {groupname}".format(groupname=groupname, cid=cid))
