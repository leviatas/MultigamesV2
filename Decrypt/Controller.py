...
async def send_codes(bot, game):
	...
	for team in [team for team in game.board.state.teams if team.clue == None]:
		await bot.send_message(team.active_player.uid, team.generate_code_message(game.groupName), ParseMode.MARKDOWN)
	await save(bot, game.cid)

async def inform_teams(bot, game):
	...
	if codigo_equipo is None and codigo_oponente is None and game.turncount != 1:
		await bot.send_message(game.cid, active_team.printHistory(), ParseMode.MARKDOWN)
	# Si ambos equipos han terminado resuelvo.
	

async def resolve(bot, game):	
	active_team = game.board.state.active_team
	...
	await bot.send_message(game.cid, "El codigo correcto era: *{}*".format(code), ParseMode.MARKDOWN)
	
	if(game.turncount != 1):
		if (intercepcion == code):		
			game.board.state.inactive_team.interceptions += 1		
			await bot.send_message(game.cid, "El equipo *{} ha interceptado* el mensaje!!!".format(game.board.state.inactive_team.name), ParseMode.MARKDOWN)
		else:
			await bot.send_message(game.cid, "Incorrecto! {} no era el código. El equipo *{} NO ha interceptado* el mensaje!!!".format(intercepcion, game.board.state.inactive_team.name), ParseMode.MARKDOWN)	
	# Se revela si ha sido desencriptado
	if (desencriptacion == code):
		await bot.send_message(game.cid, "El equipo *{} ha desencriptado* el codigo!!!".format(active_team.name), ParseMode.MARKDOWN)
	else:		
		active_team.miscommunications += 1
		await bot.send_message(game.cid, "Incorrecto! {} no era el codigo! El equipo *{} NO ha desencriptado* el codigo!!!".format(desencriptacion, active_team.name), ParseMode.MARKDOWN)	
...
async def start_next_round(bot, game):
	...
	if verify_end:
		if verify_end[0]:
			...
			await bot.send_message(game.cid, "El equipo {} ha ganado!!!".format(verify_end[1].name), ParseMode.MARKDOWN)
			await bot.send_message(game.cid, "Las palabras eran:\n\n{}\n\n{}!!!".format(palabras_team1, palabras_team2), ParseMode.MARKDOWN)
		else:
