...
`SecretHitler/` is a **self-contained** bot with its own `MainController`, `Commands`, `GamesController`, `Boardgamebox/`, `Constants/`, and DB connection. It shares only `Utils.command_status` from the root. It has its own `DBCreate.sql`, `requirements.txt`, and formerly its own Heroku/Jenkins deployment (`Procfile`, `Jenkinsfile`, `app.json`). It also has its own player stats layer: `PlayerStats.py` (per-user, per-game-type stats/achievements dict, persisted separately from game state) and `EstadisticsCalculator.py` (hypergeometric-distribution helpers for role-probability stats). At startup `main()` registers the bot's `/`-menu via `set_my_commands`, once per language (`MENU_COMANDOS` + `_menu_comandos(lang)`) — keep that list, and the `menu.<command>` keys in every locale, in sync when adding/removing commands.
...
catalog for `ctx`'s language and returns the template. **The language belongs to the chat, not to the player nor to
the game**: `/language` (alias `/idioma`, with or without an argument — the bare form opens a keyboard) stores it in
`language_secret_hitler (cid, lang)`, so it survives restarts and every `/newgame`. `i18n.init()` (called from `main()`)
preloads the whole table into a `cid -> lang` dict, so `t()` never hits the database in the hot path.

- **`ctx` is whatever you have at hand**: a `Game` (its `cid` is used), a raw `cid`, a language code, or `None`
  (falls back to Spanish). Passing a `Game` is what makes a player's *private* messages come out in the language of
  the group the game is being played in, rather than the language of the DM. Prefer `game` over `cid` whenever a game
  is in scope — `t()` accepts a `None` game, so `t("x", game)` is safe even on the branches where the game wasn't found.
- **Catalogs**: `SecretHitler/Locales/es.py` and `en.py`, each a flat `TEXTS` dict. Spanish is the default and the
  reference: a key missing in another language falls back to it (and logs), so a half-translated catalog degrades
  instead of crashing. The key sets must match — `t()` never raises, which also means a typo'd key fails silently and
  only shows up as the raw key in the chat.
- **Placeholders are part of the contract**: templates keep whatever the call site uses (`%s`/`%d` for `%`-formatting,
  `{}`/`{nombre}` for `str.format`), so a translation has to keep the same markers in the same order. Adding a key
  means adding it to *both* files.
- **Role, party and policy names are identifiers, not text**: `"Liberal"`, `"fascista"`, `"socialista"` etc. are what
  lives in the game state and in the database, and they stay Spanish forever. Only the *display* goes through
  `i18n.role_name()` / `party_name()` / `policy_name()`, and `preference_label()` for a `/role` preference like
  `"Liberal_Fascista"`. Never translate a value before storing or comparing it.
- **`Game.print_roles(ctx)` is the one place where that distinction bites**: the copy written to
...
- Telegram's `/` menu is registered once per language via `set_my_commands(..., language_code=...)`, so a player whose
  Telegram client is in English sees the English descriptions regardless of the group's setting.

#### Achievements system

- **Catalog**: `SecretHitler/Constants/Achievements.py` — a flat `LOGROS` list of `Logro` namedtuples (`code, emoji, categoria, secreto, check`). `code` is the stable key persisted in `achievements_secret_hitler_players.achievement_code`; **never rename it** once shipped, or you orphan everyone's unlocks for it — and it's also what the name and the description are looked up by (`logro.<code>.name` / `logro.<code>.desc` in every locale, read through `Achievements.nombre_logro()` / `descripcion_logro()`). `secreto=True` hides the name and the description in `/logros` until unlocked. `check(ctx)` is a pure predicate over an `Achievements.Ctx`. `categoria` must be one of `CATEGORIAS` (that list is also the display order in `/logros`), which includes `socialista` ("Expansión Socialista") for everything the expansion added. The full catalog no longer fits in one Telegram message, so `/logros` goes out through `send_chunked_message`.
...
A player can have their **Ja** registered automatically the moment a formula is proposed (`Player.auto_ja`, per game). It stops applying once the match gets serious, and **each player picks when that is** via `Player.auto_ja_corte`: `"fascistas"` (Hitler zone, 3 fascist policies), `"politicas"` (`POLITICAS_PARA_CORTAR_AUTOJA` = 5 policies enacted, counting *all* tracks — the socialist one included), or `"ambas"` (either, the default). `Constants/Config.py` keeps only the list of valid values (`CORTES_AUTOJA`) so `Player` can read it without importing the controller; the prose (`autoja.corte.<valor>`) and the button labels (`autoja.btn.<valor>`) live in the locale catalogs and are read through `Player.texto_corte_autoja()`; `Player.corte_autoja()` normalizes the field (`getattr` + validation, so games saved before this was configurable — or with a bogus value — fall back to the default). The decision itself is `MainController.autoja_cortado(game, player)`, and **every** place that auto-registers a Ja must go through it rather than re-checking `is_zona_hitler` — today that's `vote()` and the in-progress-vote branch of `Commands.set_auto_ja()`. Activating auto-Ja sends the three-option keyboard (`autojacorte` → `callback_autoja_corte`, whose callback data carries the *game's* cid so the handler can find it from a DM); ignoring those buttons just keeps the default. The setting lives on the `Player`, so it resets with every new game.
