...
async def command_pass(update: Update, context: CallbackContext):
	...
	if uid == game.board.state.active_team.active_player.uid and game.board.state.fase_actual == "Set_Reference":
		...
		await bot.send_message(game.cid, "{} decidio descartar la carta {}/{}. Se le enviara nueva carta.".format(
		...
		await WavelengthController.send_wavelength(bot, game)		
...
async def command_draw(update: Update, context: CallbackContext):
	bot = context.bot
	args = context.args
	cid = update.message.chat_id
	uid = update.message.from_user.id
...
async def callback_Wave(update: Update, context: CallbackContext):
	bot = context.bot
	callback = update.callback_query
	log.info('callback_Wave called: %s' % callback.data)
	regex = re.search(r"(-?[0-9]*)\*([A-Za-z]*)Wave\*(.*)\*(.*)", callback.data)
	...
	if accion == "LeftRight" and game.board.state.fase_actual == "Predict_Opp_LR" and (game.board.state.inactive_team.belongs(uid) or uid == ADMIN[0]):
		await command_left_right(bot, [dato1, cid, uid])
	# Change Grade action, predict phase, belong to active team, and not the active player.	
		...
		if refresh:
			await WavelengthController.draw_choose_needle(bot, game, callback.message.message_id)
...
async def callback_choose_game_prop(update: Update, context: CallbackContext):
	...
	regex = re.search(r"(-[0-9]*)\*choosegamerefWL\*(.*)\*([0-9]*)", callback.data)
...
async def add_propose(bot, game, uid, propuesta):
	...
	if uid in game.playerlist:
		...
		await bot.send_message(uid, "No puedes hacer ref si no estas en el partido.")
		
# Por async defecto no hay restricciones
async def get_choose_game_buttons(games_tipo, uid, fase_actual, button_value, callback_command):	
	btns = []
	cid = None
...
async def command_pick(bot, update, args):	
	try:
...
async def command_left_right(bot, args):
	cid, uid = args[1], args[2]
...
async def command_reference(update: Update, context: CallbackContext):
	...
	try:
		...
		if len(args) > 0:
			...
			btns, cid = await get_choose_game_buttons(games_tipo,
							    uid, 
